            ...
            <c:choose>
                <c:when test="${isLoggedin}">
                    <c:if test="${loginedUser != null}">
                        <c:forEach items="${loginedUser.roles}" var="role" varStatus="loop">
                            <c:if test="${role.id == 1}">
                                <li class="nav-item"><a class="nav-link">Posts</a></li>
                                <div class="btn-group">
                                    <button type="button"
                                        class="btn btn-secondary dropdown-toggle"
                                        data-bs-toggle="dropdown" data-bs-display="static"
                                        aria-expanded="false">
                                        <c:out value="${sessionScope.loginedUser.name}" />
                                    </button>
                ...
                <c:otherwise>
                    ...
                    <li class="nav-item"><a href="<c:url value='/register' />" class="nav-link">Get Started</a></li>
                </c:otherwise>
            </c:choose>         
        </ul>
    </div>
